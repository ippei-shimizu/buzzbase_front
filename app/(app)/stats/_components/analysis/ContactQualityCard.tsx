"use client";
import type { ContactQualityCategory } from "../../analysisActions";
...
/** 打球の質（真芯/先っぽ/詰まり/擦り/ドライブ）の内訳ドーナツ + 横棒凡例。 */
export function ContactQualityCard({
  breakdown,
  total,
}: ContactQualityCardProps) {
  if (total === 0) {
    return (
      <section className="rounded-xl bg-[#3A3A3A] p-4">
        <h3 className="text-base font-bold text-[#F4F4F4]">打球の質</h3>
  ...
  return (
    <section className="rounded-xl bg-[#3A3A3A] p-4">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-base font-bold text-[#F4F4F4]">打球の質</h3>
        <span className="text-[11px] text-[#A1A1AA]">対象 {total} 打球</span>
      </div>
