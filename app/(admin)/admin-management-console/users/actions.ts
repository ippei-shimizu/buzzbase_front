...
export async function createAdminUser(
  ...
  try {
    ...
    if (!response.ok) {
      return {
        success: false,
        errors: data.errors || ["管理者ユーザーの作成に失敗しました"],
      };
    }
...
export async function updateAdminUser(
  ...
  try {
    ...
    const response = await fetch(
      `${RAILS_API_URL}/api/v1/admin/admin_users/${id}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!response.ok) {
      return {
        success: false,
        errors: data.errors || ["管理者ユーザーの更新に失敗しました"],
      };
    }
...
export async function deleteAdminUser(
  ...
  try {
    ...
    if (!response.ok) {
      return {
        success: false,
        errors: data.errors || ["管理者ユーザーの削除に失敗しました"],
      };
    }
...
export async function createAdminUserAction(formData: FormData) {
  ...
  if (result.success) {
    ...
    redirect(
      `/admin-management-console/users?mode=create&error=${encodeURIComponent(result.errors?.join(", ") || "作成に失敗しました")}`,
    );
  }
}
...
export async function updateAdminUserAction(id: number, formData: FormData) {
  ...
  if (result.success) {
    ...
    // エラーがある場合は、エラー情報と共にフォームを再表示
    redirect(
      `/admin-management-console/users?mode=edit&id=${id}&error=${encodeURIComponent(result.errors?.join(", ") || "更新に失敗しました")}`,
    );
  }
}
