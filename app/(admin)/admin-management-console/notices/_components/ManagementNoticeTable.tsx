...
interface ManagementNoticeTableProps {
  notices: ManagementNotice[];
}

export default function ManagementNoticeTable({
  notices,
}: ManagementNoticeTableProps) {
  if (notices.length === 0) {
    return (
      <div className="bg-white shadow rounded-lg p-6">
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">新規お知らせを作成してください。</p>
        </div>
      </div>
    );
  }
              ...
              >
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <div className="flex justify-end space-x-2">
                    <Link
                      href={`/admin-management-console/notices?mode=edit&id=${notice.id}`}
                    ...
                    <Link
                      href={`/admin-management-console/notices?mode=delete&id=${notice.id}`}
