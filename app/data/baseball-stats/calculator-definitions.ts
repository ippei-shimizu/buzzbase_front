...
export const calculatorDefinitions: Record<string, CalculatorDefinition> = {
  ...
  era: {
    ...
    title: "防御率計算ツール",
    metaTitle:
      "防御率（ERA）計算ツール【無料・登録不要】｜自責点と投球回を入力→即計算",
    metaDescription:
      "自責点と投球回を入力するだけで防御率（ERA）を瞬時に計算。プロ野球・高校野球での目安値（2.00以下がエース級など）も解説。登録不要・無料。投手成績の管理は BUZZ BASE アプリで自動算出も可能です。",
  ...
  "winning-percentage": {
    ...
    title: "勝率計算ツール",
    metaTitle:
      "勝率計算ツール｜勝利数と敗北数を入力するだけで勝率を自動計算【登録不要・無料】",
    metaDescription:
      "勝利数と敗北数を入力するだけで勝率を自動計算。登録不要・無料でブラウザからすぐ使えます。勝率の計算式・目安値・意味もわかりやすく解説。NPB・MLBでの引き分けの扱いや勝率の基準も紹介。",
    heading: "勝率計算ツール",
    description:
